...
TEST(SortTest, testSortByRatingDescending) {
	...
	for (unsigned i = 1; i < MovieDatabase.returnSortedList().size(); ++i) {
		if (MovieDatabase.returnSortedList().at(i).getRating() > MovieDatabase.returnSortedList().at(i - 1).getRating()) {
			FAIL() << "Movies are not correctly sorted by rating in descending order.";
		}
	}
	SUCCEED() << "Movies are correctly sorted by rating in descending order.";
}
...
TEST(SortTest, testSortByRatingAscending) {
    ...
    for (unsigned i = 1; i < MovieDatabase.returnSortedList().size(); ++i) {
		if (MovieDatabase.returnSortedList().at(i).getRating() < MovieDatabase.returnSortedList().at(i - 1).getRating()) {
			FAIL() << "Movies are not correctly sorted by rating in ascending order.";
		}
	}
...
TEST(SortTest, testSearchByRatings) {
	...
	for (unsigned i = 0; i < MovieDatabase.returnSortedList().size(); ++i) {
		if (MovieDatabase.returnSortedList().at(i).getRating() < 4.0 || MovieDatabase.returnSortedList().at(i).getRating() > 4.5) {
			FAIL() << "Movies are not correctly filtered by rating within the search range.";
		}
	}
	SUCCEED() << "Movies are correctly filtered by rating within the search range and sorted by rating in ascending order.";
}
...
TEST(SortTest, testSortAscendingTitles) {
	...
	for (unsigned i = 1; i < MovieDatabase.returnSortedList().size(); ++i) {
		if (MovieDatabase.returnSortedList().at(i).getTitle() < MovieDatabase.returnSortedList().at(i - 1).getTitle()) {
			FAIL() << "Movies are not correctly sorted by title in ascending order.";
		}
	}
	SUCCEED() << "Movies are correctly sorted by title in ascending order.";
}
...
TEST(SortTest, testSortDescendingTitles) {
    ...
    for (unsigned i = 1; i < MovieDatabase.returnSortedList().size(); ++i) {
		if (MovieDatabase.returnSortedList().at(i).getTitle() > MovieDatabase.returnSortedList().at(i - 1).getTitle()) {
			FAIL() << "Movies are not correctly sorted by title in descending order.";
		}
	}
	SUCCEED() << "Movies are correctly sorted by title in descending order.";
}
...
TEST (SortTest, testSearchMovieTitle){
	...
	for (unsigned i = 0; i < MovieDatabase.returnSortedList().size(); ++i) {
		...
		for (unsigned j = 0; j < MovieDatabase.returnSortedList().at(i).getTitle().size() - 5; ++j) {
			string subStr = MovieDatabase.returnSortedList().at(i).getTitle().substr(j, 5);
		...
		if (!matchesSearch) {
			FAIL() << "Movies are not correctly filtered by title given the search term.";
		}
	}
	SUCCEED() << "Movies are correctly filtered by title given the search term and sorted by title in ascending order.";
}
...
TEST(SortByGenres, searchByGenre_Genre){
    ...
    for (unsigned i = 0; i < genreMovieList.size(); ++i) {
		bool matchesGenre = false;
		for (unsigned j = 0; j < genreMovieList.at(i).getGenreList().size(); ++j) {
			if (genreMovieList.at(i).getGenreList().at(j) == "Mystery") {
				matchesGenre = true;
			}
		}
		if (!matchesGenre) {
			FAIL() << "Movies are not correctly filtered by genre given the genre.";
		}
    }
	SUCCEED() << "Movies are correctly filtered by genre given the genre.";
}
...
TEST(SortByGenres, searchByGenre_int){
    ...
    for (unsigned i = 0; i < genreMovieList.size(); ++i) {
		bool matchesGenre = false;
		for (unsigned j = 0; j < genreMovieList.at(i).getGenreList().size(); ++j) {
			if (genreMovieList.at(i).getGenreList().at(j) == "Adventure") {
				matchesGenre = true;
			}
		}
		if (!matchesGenre) {
			FAIL() << "Movies are not correctly filtered by genre given the corresponding number.";
		}
    }
	SUCCEED() << "Movies are correctly filtered by genre given the corresponding number.";
}
